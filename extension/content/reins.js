/**
 * Content script for REINS (reins.or.jp).
 *
 * ⚠️ REINS は業者専用サイトです。ログイン後に実際のフォームHTMLを確認し、
 *    sites_config.js の reins.fields セレクタを更新してください。
 */
...
(function () {
  ...
  function fillForm(prop, config) {
    ...
    // Area (extract numeric value from "XX.XXm²")
    const areaEl = queryAny(config.fields.area);
    const areaNum = prop.area?.replace(/[^\d.]/g, "") || "";
    if (fillInput(areaEl, areaNum)) count++;

    // Build year (extract year from "築XX年" → calculate approximate year)
    const buildYearEl = queryAny(config.fields.buildYear || []);
    if (buildYearEl && prop.age) {
      const ageMatch = prop.age.match(/(\d+)年/);
      if (ageMatch) {
        const builtYear = new Date().getFullYear() - parseInt(ageMatch[1]);
        fillInput(buildYearEl, String(builtYear));
        count++;
      }
    }
